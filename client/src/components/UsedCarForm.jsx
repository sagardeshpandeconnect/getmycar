...
//     if (encodedId) {
//       try {
//         // Send a request to delete the existing image from Cloudinary
//         await fetch(`/usedcars/deleteimage/${encodedId}`, {
//           method: "DELETE",
//         });
//       } catch (error) {
...
//         }
//       } else {
//         const response = await postData(`/usedcars/upload`, processedData);
...
const UsedCarForm = () => {
  ...
  const handlePictureChange = async (e) => {
    ...
    if (pictureId) {
      try {
        // Use POST method to delete the existing image
        const response = await fetch(`/usedcars/deleteimage`, {
          method: "POST",
          headers: {
